...
class MorfoCuencasDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    def Pbtn2_clicked(self): #evento en Boton PushButton
        if (self.iniciar > 0):
            ...
            alturas=int(self.lneAlturas.text())
            ...
            self.Proceso2(entrada,directorioTrabajo, umbral, distancia,self.CoordenadasPC, alturas)  #Invoque la función Proceso
        else:
    ...
    def Proceso2(self, capa, directorio, param_umbral, param_distancia,coordenadas, alturas):  #modulo proceso desencadenado en boton PushButton
        #processing.algorithmHelp("grass7:r.fill.dir")
        #directorio='D:/UD_SIG/PGRADO/Salida/'
        umbral = param_umbral
        ...
        poligono_divisoria= directorio+'divisoria_cuencas_poly.shp'
        poly_divisoria=QgsVectorLayer(poligono_divisoria, "poligono")
        #Si genera multipartes
        if (poly_divisoria.featureCount()>1):
            poly_divisoria.selectAll()
            lista_area=[]
            ids_poly=[]
            selfea_poly=poly_divisoria.selectedFeatures()
            for poly in selfea_poly:
                    lista_area.append(poly.geometry().area())
                    ids_poly.append(poly.id())
            
            index_poly=lista_area.index(max(lista_area))
            poly_divisoria.startEditing()
            for poly in selfea_poly:
                id_p=poly.id()
                if (id_p != ids_poly[index_poly]):
                    poly_divisoria.deleteFeature(poly.id())
                    #poly_divisoria.deleteFeature(poly.id())
            poly_divisoria.commitChanges()       
        ...
        ####Aqui empieza proceso de calculo de orden #OJO ARREGLO DAVID FIX
        ###FIX GEOMETRIES###
        #processing.algorithmHelp("native:fixgeometries")
        cuenca_poly = directorio + 'divisoria_cuencas_poly.shp'
        cuenca_polyf = directorio + 'divisoria_cuencas_polyf.shp'
        ...
        cuenca_poly = directorio + 'divisoria_cuencas_polyf.shp'
